...
*/
void run_iat(const char* audio_file, const char* output_file, const char* session_begin_params)
{
	...
	//printf("%s\n",rec_result);
	//printf("=============================================================\n");
        /*本部分实现将转换文本写入write_text.txt*/
        FILE* fstream;
        char*ss = rec_result;
        int i;
        fstream = fopen(output_file,"w");
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	char filename[64];
	char output_file[64];//could not too large will cause :write_text.txt�v
	if(argc == 3)
	{
		strncpy(filename,argv[1],strlen(argv[1]));
		strncpy(output_file,argv[2],strlen(argv[2]));
	...
	run_iat(filename, output_file, session_begin_params); //语音识别音频文件
exit:
